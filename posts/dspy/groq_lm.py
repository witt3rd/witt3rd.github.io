...
class GroqLM(LM):
    def __init__(
        self, model=LLAMA3_MODEL, **kwargs
    ):  # model="mixtral-8x7b-32768", **kwargs):
...
if __name__ == "__main__":
    init_dspy(GroqLM, model=LLAMA3_MODEL, max_tokens=2000)
    # init_dspy(max_tokens=2000)
    pred = dspy.Predict("prompt -> code")(
        prompt="Fast API CRUD endpoint for fire alarm global IoT network"
    )
    print(pred.code)
